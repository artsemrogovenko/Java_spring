...
public class WebService {
    /**
     * Объект клиента Feigen для запросов записи задач .
     */
    private final AccountApi accountApi;
    /**
     * Объект клиента Feigen для доски задач.
     */
    ...
    public List<Task> getAll() {//показать только доступные мне задачи TODO доработать
        return storageApi.getTasks();
//        return storageApi.getTasks().stream()
//                .filter(task -> task.getOwner() != null && task.getOwner().equals(1L)).collect(Collectors.toList());
    }
